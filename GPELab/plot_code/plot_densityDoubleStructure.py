...
densities1 = np.ones((n_Om, len_sim)) #matrix for all the Rabi 
densities2 = np.ones((n_Om, len_sim)) #matrix for all the Rabi


mat = scipy.io.loadmat(r'C:\Users\Sarah\Documents\GitHub\Numerical-simulations\GPELab\outputs\doubleStructure\new.mat')
...
figS, axS = plt.subplots(1,1,constrained_layout=True, figsize=(17,4))

#for i in range(np.size(Omega_values)):
for i in range(n_Om):
    axS.plot(densities1[i,:], label=labels[0], lw=1.7, color=colors[0],zorder =1)
    axS.plot(densities2[i,:], label=labels[1], lw=1.7, color=colors[1],zorder =1)
