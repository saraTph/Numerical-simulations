...
# delta_sweep[3,:], kinEn_sweep[3,:] = np.genfromtxt(file, delimiter='\t', skip_header=1, comments='#', unpack=True)

file = os.path.join(location, 'kinEn_sweep_4.txt')
delta_sweep[4,:], kinEn_sweep[4,:] = np.genfromtxt(file, delimiter='\t', skip_header=1, comments='#', unpack=True)

# file = os.path.join(location, 'kinEn_sweep_5.txt')
# delta_sweep[5,:], kinEn_sweep[5,:] = np.genfromtxt(file, delimiter='\t', skip_header=1, comments='#', unpack=True)
...
idx = int(np.where(delta_sweep == 0.25)[0][0])
...
ax.scatter(Omega_values, time_FORT2off *1e3, lw=lw, color=colors[0], zorder=1)
ax.set_xlabel(r'$\Omega$ (Hz)', fontsize=14)  # Time on bottom
ax.set_ylabel(r'$t (s)$', fontsize=14)
